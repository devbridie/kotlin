...
class ReplaceWithEnumMapInspection : AbstractKotlinInspection() {
    override fun buildVisitor(holder: ProblemsHolder, isOnTheFly: Boolean): PsiElementVisitor {
        return callExpressionVisitor { element ->
            if (element.platform !is JvmPlatform) return@callExpressionVisitor
            val context = element.analyze()
            val fqName = element.getResolvedCall(context)?.resultingDescriptor?.fqNameUnsafe?.asString() ?: return@callExpressionVisitor
            if (!hashMapCreationFqNames.contains(fqName)) return@callExpressionVisitor
            if (element.valueArguments.size != 0) return@callExpressionVisitor

            val expectedType = context[BindingContext.EXPECTED_EXPRESSION_TYPE, element] ?: return@callExpressionVisitor
            val firstArgType = expectedType.arguments.firstOrNull()?.type ?: return@callExpressionVisitor
            if (!firstArgType.isEnum()) return@callExpressionVisitor
            val enumClassName = firstArgType.constructor.declarationDescriptor?.fqNameUnsafe?.asString() ?: return@callExpressionVisitor
